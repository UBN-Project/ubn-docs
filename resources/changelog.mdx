...
### 2026-04-26 — Billing & Wallet System

- **New:** Prepaid pay-as-you-go billing with lien-capture-release
- **New:** Wallet automatically created on go-live approval with permanent Virtual Account
- **New:** `GET /api/v1/billing/wallet` — view wallet balance and VA funding details
- **New:** `GET /api/v1/billing/statement` — paginated wallet statement with endpoint tracking
- **New:** `GET /api/v1/billing/pricing` — current per-call pricing table
- **New:** `402 Payment Required` response when wallet balance is insufficient
- **Changed:** Base URLs updated: `dev-api.onecluster.co` (sandbox) and `api.onecluster.co` (production)
...
<AccordionGroup>
  ...
  <Accordion title="Sandbox Environment">
    ...
    - Sandbox base URL: `https://dev-api.onecluster.co`
