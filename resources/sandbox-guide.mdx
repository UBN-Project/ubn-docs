...
**Sandbox base URL:**
```
https://dev-api.onecluster.co
```

<Info>
  Your sandbox API key only works with the sandbox base URL. Your production API key only works with the production base URL (`https://api.onecluster.co`). There is no way to mix them up accidentally — the wrong key will receive a `401 Unauthorized` response.
</Info>
...
1. **Change the base URL** from `https://dev-api.onecluster.co` to `https://api.onecluster.co`.
