...
2. Go to [dev-api.onecluster.co/verify-email](https://dev-api.onecluster.co/verify-email)
...
```bash
curl -X POST https://dev-api.onecluster.co/api/Auth/verifyEmailCode \
...
<AccordionGroup>
  ...
  <Accordion title="The code says it has expired">
    ...
    ```bash
    curl -X POST https://dev-api.onecluster.co/api/Auth/resendVerificationCode \
      -H "Content-Type: application/json" \
  ...
  <Accordion title="The code says it has already been used">
    ...
    - Log in to the portal at [dev-api.onecluster.co](https://dev-api.onecluster.co)
