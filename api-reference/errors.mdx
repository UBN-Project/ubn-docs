...
| Code | HTTP Status | What happened | How to fix |
|------|-------------|---------------|------------|
| `ENVIRONMENT_MISMATCH` | 403 | You used a sandbox key (`ubn_sb_`) against the production URL, or a production key (`ubn_pk_`) against the sandbox URL. | Check your base URL and key prefix. Sandbox keys must only be used with `https://dev-api.onecluster.co`. Production keys must only be used with `https://api.onecluster.co`. |
...
| Code | HTTP Status | What happened | How to fix |
|------|-------------|---------------|------------|
| `E402` | 402 | Your wallet does not have enough available balance to cover the cost of this API call. | Fund your wallet via your Virtual Account, then retry. See [Billing & Wallet](/products/billing). |
| `WALLET_SUSPENDED` | 402 | Your wallet has been temporarily suspended by an administrator. You cannot make billable API calls until it is unsuspended. | Contact your UBN account manager or email [baas-support@unionbank.ng](mailto:baas-support@unionbank.ng) to resolve the suspension. You can still fund your wallet while suspended. |
| `WALLET_CLOSED` | 402 | Your wallet has been permanently closed. No API calls or funding are possible. | Contact your UBN account manager. A closed wallet cannot be reopened. |
...
const response = await fetchWithRetry(
  'https://dev-api.onecluster.co/api/v1/accounts',
  {
    headers: {
...
You can check real-time service status at [dev-api.onecluster.co/status](https://dev-api.onecluster.co/status).
