...
|--------|-------------|----------|
| `ubn_sb_` | Sandbox | `https://api-partner.onecluster.co` |
| `ubn_pk_` | Production | `https://api.onecluster.co` |
...
<Warning>
  Never hard-code your API key in source code. Store it in an environment variable (`UBN_API_KEY`) and read it at runtime. If you accidentally commit a key to a public repository, revoke it immediately from the [API Keys page](https://ubn-ui.onecluster.co/dashboard/settings/api-credentials) and generate a new one.
</Warning>
...
<Steps>
  ...
  <Step title="Compute an HMAC-SHA256 hash">
    Hash the JSON string using HMAC-SHA256 with your **signing secret** as the key. Your signing secret is separate from your API key. You receive it during onboarding and it's also available in the [API Keys page](https://ubn-ui.onecluster.co/dashboard/settings/api-credentials) of your dashboard.
  </Step>
...
```bash
curl https://api.onecluster.co/api/v1/payments/transfer \
  --cert client.crt \
  --key client.key \
...
const response = await fetch('https://api.onecluster.co/api/v1/payments/transfer', {
  method: 'POST',
...
<Info>
  mTLS is enforced at the network layer, before your request is processed. If the certificate is missing or invalid, the connection is terminated before the API key is checked. You will see a TLS handshake error, not an HTTP error response.
</Info>
...
- The `X-Idempotency-Replayed: true` response header tells you when a response is a replay. Log it: it means your retry worked and the original response is intact.
...
<Tip>
  In a high-reliability system, store each idempotency key alongside the result in your own database before considering the operation complete. If your process crashes before storing the result, retry with the same key, get the same response back, and then store it safely.
</Tip>
